...
libooc-dirs	:= $(patsubst %/,%,$(filter %/,$(init-y) \
			$(core-y) $(libs-y) $(drivers-y)))
libooc-alldirs := $(sort $(libooc-dirs))

init-y		:= $(patsubst %/, %/built-in.o, $(init-y))
core-y		:= $(patsubst %/, %/built-in.o, $(core-y))
drivers-y	:= $(patsubst %/, %/built-in.o, $(drivers-y))
libs-y		:= $(patsubst %/, %/libooc.a, $(libs-y))

#Build pvshema
#------------------------------------------------------------------------------

libooc-init := $(init-y)
libooc-main := $(core-y) $(libs-y) $(drivers-y)
libooc-all  := $(libooc-init) $(libooc-main)
export KBUILD_PVSCHEMA_OBJS := $(libooc-all)

lib.a: $(libooc-init) $(libooc-main)

$(sort $(libooc-init) $(libooc-main)): $(libooc-dirs) ;
...
clean-dirs	:= $(addprefix _clean_, . $(libooc-alldirs))
